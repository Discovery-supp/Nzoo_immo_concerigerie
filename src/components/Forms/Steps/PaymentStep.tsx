...
const PaymentStep: React.FC<PaymentStepProps> = ({
  ...
  const packages = [
    { id: 'basic', name: 'Basique', commission: 15 },
    { id: 'standard', name: 'Standard', commission: 12 },
    { id: 'premium', name: 'Premium', commission: 8 },
    { id: 'vip', name: 'VIP', commission: 5 }
  ];
  ...
  const handlePackageChange = (packageId: string) => {
    ...
    if (selectedPkg) {
      ...
      onCommissionRateChange(selectedPkg.commission);
    }
  };
            ...
            {packages.map(pkg => (
              <label key={pkg.id} className="flex items-center p-4 border border-light-gray rounded-lg cursor-pointer hover:border-primary transition-colors">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">Commission: {pkg.commission}%</p>
                </div>
              </label>
            ))}
          </div>
